...
func (m *StubMerkleTree) AppendLeaf(hash [32]byte) (uint64, error) {
	...
	pos := uint64(len(m.leaves)); m.leaves = append(m.leaves, hash); return pos, nil
}

func (m *StubMerkleTree) InclusionProof(position, treeSize uint64) (*types.MerkleProof, error) {
	m.mu.Lock(); defer m.mu.Unlock()
	if position >= uint64(len(m.leaves)) { return nil, errors.New("position out of range") }
	if treeSize > uint64(len(m.leaves)) { return nil, errors.New("tree size exceeds current size") }
...
func (m *StubMerkleTree) Head() (types.TreeHead, error) {
	m.mu.Lock(); defer m.mu.Unlock()
	if len(m.leaves) == 0 { return types.TreeHead{}, nil }
...
func (m *StubMerkleTree) computePath(position, treeSize uint64) [][32]byte {
	if treeSize <= 1 { return nil }
	var siblings [][32]byte
	hashes := make([][32]byte, treeSize); copy(hashes, m.leaves[:treeSize])
	for len(hashes) > 1 {
		sibIdx := position ^ 1
		if sibIdx < uint64(len(hashes)) { siblings = append(siblings, hashes[sibIdx]) }
		newHashes := make([][32]byte, (len(hashes)+1)/2)
		for i := 0; i < len(hashes); i += 2 {
			if i+1 < len(hashes) {
				var combined [64]byte
				copy(combined[0:32], hashes[i][:]); copy(combined[32:64], hashes[i+1][:])
				newHashes[i/2] = sha256.Sum256(combined[:])
			} else { newHashes[i/2] = hashes[i] }
		}
		hashes = newHashes; position /= 2
	}
	return siblings
}

func (m *StubMerkleTree) computeRoot(start, end uint64) [32]byte {
	if end-start == 0 { return [32]byte{} }
	if end-start == 1 { return m.leaves[start] }
	hashes := make([][32]byte, end-start); copy(hashes, m.leaves[start:end])
	for len(hashes) > 1 {
		newHashes := make([][32]byte, (len(hashes)+1)/2)
		for i := 0; i < len(hashes); i += 2 {
			if i+1 < len(hashes) {
