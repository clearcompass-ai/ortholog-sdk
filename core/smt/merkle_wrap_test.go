...
// TestStubMerkleTree_InclusionProofRoundTrip closes the end-to-end
// loop: a proof generated by the stub must verify via the SMT
// package's VerifyMerkleInclusion against the same tree's Head().
// This is the test that would have caught the prefix-mismatch bug
// where generator and verifier disagree on interior hashing.
func TestStubMerkleTree_InclusionProofRoundTrip(t *testing.T) {
	tree := smt.NewStubMerkleTree()
	for _, data := range [][]byte{[]byte("a"), []byte("b"), []byte("c"), []byte("d")} {
		if _, err := tree.AppendLeaf(data); err != nil {
			t.Fatal(err)
		}
	}
	head, _ := tree.Head()
	for pos := uint64(0); pos < head.TreeSize; pos++ {
		proof, err := tree.InclusionProof(pos, head.TreeSize)
		if err != nil {
			t.Fatalf("InclusionProof(%d): %v", pos, err)
		}
		if err := smt.VerifyMerkleInclusion(proof, head.RootHash); err != nil {
			t.Errorf("round-trip verify at position %d: %v", pos, err)
		}
	}
}
