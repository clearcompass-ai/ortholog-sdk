...
func DefaultHash(depth int) [32]byte {
	if depth < 0 || depth > TreeDepth { return [32]byte{} }
...
func (t *Tree) StartTracking() {
	t.mutMu.Lock(); defer t.mutMu.Unlock()
	t.trackMutations = true; t.mutations = nil
}

func (t *Tree) StopTracking() []types.LeafMutation {
	t.mutMu.Lock(); defer t.mutMu.Unlock()
	t.trackMutations = false
	result := t.mutations; t.mutations = nil; return result
}
...
func (t *Tree) SetLeaf(key [32]byte, leaf types.SMTLeaf) error {
	if t.trackMutations {
		...
		if old != nil { mut.OldOriginTip = old.OriginTip; mut.OldAuthorityTip = old.AuthorityTip }
...
func (t *Tree) Root() ([32]byte, error) {
	...
	if count == 0 { return defaultHashes[TreeDepth], nil }
	return t.computeRoot()
}

func (t *Tree) computeRoot() ([32]byte, error) {
	store, ok := t.leaves.(*InMemoryLeafStore)
	if !ok { return defaultHashes[TreeDepth], nil }
	if len(store.store) == 0 { return defaultHashes[TreeDepth], nil }
	leafHashes := make(map[[32]byte][32]byte)
	for key, leaf := range store.store { leafHashes[key] = hashLeaf(leaf) }
...
func hashLeaf(leaf types.SMTLeaf) [32]byte {
	...
	b := make([]byte, 8)
	for i := 0; i < 8; i++ { b[i] = byte(leaf.OriginTip.Sequence >> (56 - uint(i)*8)) }
	data = append(data, b...)
	data = append(data, []byte(leaf.AuthorityTip.LogDID)...)
	for i := 0; i < 8; i++ { b[i] = byte(leaf.AuthorityTip.Sequence >> (56 - uint(i)*8)) }
	data = append(data, b...)
...
func computeSparseRoot(leafHashes map[[32]byte][32]byte, depth int) [32]byte {
	if depth == 0 {
		if len(leafHashes) == 0 { return defaultHashes[0] }
		for _, h := range leafHashes { return h }
	}
	bitIdx := uint(TreeDepth - depth)
	left := make(map[[32]byte][32]byte)
	right := make(map[[32]byte][32]byte)
	for key, hash := range leafHashes {
		byteIdx := bitIdx / 8; bitMask := byte(0x80 >> (bitIdx % 8))
		if key[byteIdx]&bitMask == 0 { left[key] = hash } else { right[key] = hash }
	}
	var leftHash, rightHash [32]byte
	if len(left) == 0 { leftHash = defaultHashes[depth-1] } else { leftHash = computeSparseRoot(left, depth-1) }
	if len(right) == 0 { rightHash = defaultHashes[depth-1] } else { rightHash = computeSparseRoot(right, depth-1) }
	var combined [64]byte
	copy(combined[0:32], leftHash[:]); copy(combined[32:64], rightHash[:])
...
func (s *InMemoryLeafStore) Get(key [32]byte) (*types.SMTLeaf, error) {
	...
	leaf, ok := s.store[key]; if !ok { return nil, nil }; return &leaf, nil
}
func (s *InMemoryLeafStore) Set(key [32]byte, leaf types.SMTLeaf) error {
	s.mu.Lock(); defer s.mu.Unlock(); s.store[key] = leaf; return nil
}
func (s *InMemoryLeafStore) Delete(key [32]byte) error {
	s.mu.Lock(); defer s.mu.Unlock(); delete(s.store, key); return nil
}
func (s *InMemoryLeafStore) Count() (int, error) {
	s.mu.RLock(); defer s.mu.RUnlock(); return len(s.store), nil
}
...
func (c *InMemoryNodeCache) Get(key [32]byte) ([]byte, bool) {
	c.mu.RLock(); defer c.mu.RUnlock(); v, ok := c.store[key]; return v, ok
}
func (c *InMemoryNodeCache) Set(key [32]byte, value []byte) {
	c.mu.Lock(); defer c.mu.Unlock(); c.store[key] = value
}
