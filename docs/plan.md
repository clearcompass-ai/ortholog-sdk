...
**Subgroup 1.1 — `LengthPrefixed` primitive.** Add `LengthPrefixed(dst string, fields ...[]byte) [32]byte` to `crypto/hash.go`. Implementation writes, in order: 2-byte big-endian length of DST, DST bytes, then for each field: 2-byte big-endian length, field bytes. Final SHA-256 digest. Zero interpretation of field contents — raw bytes in, raw bytes hashed.

Godoc explicitly states three things:

1. **The TupleHash discipline.** Length-prefix everything. No exceptions at the call-site level — any SDK identifier that hashes variable-length inputs routes through this helper. Raw-concatenation constructions for SplitIDs, commitment hashes, transcript inputs, and domain-app identifiers are forbidden.

2. **The RFC 9380 carveout, stated with the correct boundary.** The only strings that skip this helper are the IETF-standardized hash-to-curve suite IDs embedded inside `expand_message_xmd` — the DST format defined in RFC 9380 §3.1, applied during BLS12-381 point derivation. Those suite IDs (e.g., `BLS_SIG_BLS12381G2_XMD:SHA-256_SSWU_RO_NUL_`) are governed by the IETF specification and interop with external BLS implementations depends on using them exactly as the RFC specifies. They do not route through `LengthPrefixed`.

   **Ortholog-bespoke tags are NOT carveouts.** The cosignature domain tag and the Proof-of-Possession domain tag used in `crypto/signatures/bls_verifier.go` are application-layer DSTs unique to this protocol. They namespace Ortholog-internal BLS signatures against cross-protocol confusion, and they get composed with variable-length Ortholog fields (entry hashes, tree head hashes, signer identifiers) at signing and verification time. If those compositions are raw-concatenated, the BLS verification path inherits the exact boundary-shift collision class the universal rule exists to prevent. The cosignature domain tag and PoP domain tag MUST migrate to `LengthPrefixed`; this migration is scoped to Group 5 Subgroup 5.3. The godoc names them explicitly as migration targets (not exceptions) to prevent future drift.

3. **The caller-normalizes contract.** NFC normalization of DIDs and other Unicode strings happens at the edge where the caller enters the SDK, never inside this function. The helper operates on raw bytes; if two callers pass different byte sequences for what they consider "the same" DID, they get different hashes. This is by design — the SDK does not guess at caller intent.

The 2-byte length caps field size at 65535 bytes; exceeding it panics because any legitimate cryptographic identifier fits comfortably under this bound and a >65KB field is a caller bug.
...
- `TestLengthPrefixed_BoundaryShift` pins that `LengthPrefixed("DA", []byte("lice"))` ≠ `LengthPrefixed("D", []byte("Alice"))` — the boundary-shifting attack the universal rule exists to prevent.

- Four golden vectors with pinned 32-byte digests cover: empty DST + empty field, empty DST + standard field, standard DST + empty field, and the canonical PRE case with realistic DIDs and a SHA-256 CID.

- `TestCID_BytesIncludesAlgorithmTag` (in the newly created `storage/cid_test.go`) constructs a CID and verifies `Bytes()[0]` equals the algorithm byte. A second assertion verifies two CIDs with identical 32-byte digests under different hypothetical algorithm tags produce distinct `Bytes()` outputs.

- `TestLengthPrefixed_RFCCarveoutDocumented` is a runtime `go/ast` or regex regression test that parses the `crypto/hash.go` source and verifies the godoc contains the RFC 9380 boundary statement with the correct orientation: IETF suite IDs are the carveout, Ortholog-bespoke tags migrate. The test fails if a future edit inverts the boundary or drops the named-target-vs-exception distinction. This is load-bearing specifically because the inverted wording would document a vulnerability as policy; the test exists to catch that class of error before it reaches review.

**Verification for Group 1.** `go test ./crypto/...` and `go test ./storage/...` green. Four golden vectors locked in testdata. `storage/cid_test.go` created and passing. One-line note added to `docs/crypto.md` cross-referencing the rule for downstream readers, including the correct RFC 9380 boundary. `grep -rn 'sha256.Sum256' crypto/ core/` reviewed to identify any site that hashes variable-length fields without length-prefixing — if found, either migrated in later groups or documented as an intentional exception (e.g., RFC 6962 leaf hash in `core/envelope/` which is protocol-mandated).

---

## Group 2 — SplitID Migration + Schema Parameters v7.5 Verification

**Packages:** `crypto/artifact/`, `crypto/escrow/vss_v2.go`, `schema/parameters_json.go` (verification only, no code change), `schema/parameters_json_roundtrip_test.go` (test additions only), `types/schema_parameters.go`

**Purpose:** Apply the universal length-prefix rule to both SplitID derivations — one new (PRE), one migrated (escrow) — and lock the v7.5 schema-parameters restructure by adding the missing test surface around its fail-closed enum branches and sentinel semantics. The legacy `ComputeEscrowSplitID` construction in `vss_v2.go` uses raw concatenation of `(DST, BE_uint16(len(DID)), DID, nonce)`. That construction breaks the universal rule (the DST and nonce are not length-prefixed). The migration is a hard break — every SplitID produced by `ComputeEscrowSplitID` changes — and because Phase C is pre-production, this is the correct time to absorb it. ADR-005 §2 formalizes the canonical construction for both SplitIDs.
...
**Subgroup 2.3 — Schema parameters v7.5 lock: verification plus targeted test additions.** Evidence from `schema/parameters_json.go` and `builder/entry_builders.go`: the v7.5 restructure is complete in source. The verification portion of this subgroup confirms that completion; the engineering portion adds the test surface that makes the fail-closed discipline and the sentinel semantics visible in the test manifest.

*What is already shipped and verified in-source (no code change).* `BuildSchemaEntry` takes `types.SchemaParameters` and marshals via `schema.MarshalParameters`; it is the sole schema-definition entry producer in the SDK. The `jsonSchemaPayload` struct (Extract side) and `marshalShape` struct (Marshal side) mirror each other field-for-field across all 13 well-known fields. Option A discipline (unconditional scalar emission) is implemented in `MarshalParameters` and documented on `marshalShape`. The `-1` sentinel for `CredentialValidityPeriod` nil is implemented as a mirrored round-trip via the `credentialValidityPeriodNilSentinel` constant, with semantics documented at both the declaration site and the Extract branch that consumes it. The empty-string sentinel for unset `MigrationPolicy` is implemented symmetrically. `CommutativeOperations` emits `[]` never `null` via the explicit non-nil slice assignment in Marshal and the non-nil default in Extract. All four enums (`MigrationPolicy`, `ArtifactEncryption`, `GrantAuthorizationMode`, `OverrideThreshold`) have fail-closed `default` branches on both Extract and Marshal sides that return explicit "unknown value" errors.

*What confirms the verification.* `grep -rn 'json.Marshal(map\[string\]any{.*cosignature_threshold' --include='*.go'` returns zero hits. `grep -rn 'json.Marshal(map\[string\]any{.*activation_delay' --include='*.go'` returns zero hits. The four `json.Marshal(map[string]any{...})` call sites that remain in `builder/entry_builders.go` are in `BuildAnchorEntry`, `BuildKeyRotation`, `BuildKeyPrecommit`, and `BuildMirrorEntry` — none of which produce schema-definition entries and all of which have domain-opaque payloads per the builder package's architectural contract ("Domain Payload is opaque to builder" per the package godoc). `parameters_json_roundtrip_test.go` exists and is named in `MarshalParameters`' godoc as the permanent regression gate; its exhaustive coverage of all 13 fields, both sentinel cases, and the `CommutativeOperations` empty-vs-populated distinction is the condition under which the v7.5 restructure was considered complete. Subgroup 2.3 does not re-verify that exhaustive coverage by hand — the existence of the test at its named path, with the field set the godoc enumerates, is the closure of the Extract↔Marshal round-trip.

*What is added by this subgroup.* Eight named tests that make the fail-closed discipline and the sentinel semantics visible as distinct assertions, each failing for a distinct reason if the corresponding guarantee regresses:

- `TestMarshalParameters_RejectsUnknownEnum_MigrationPolicy` constructs `types.SchemaParameters{MigrationPolicy: 99}`, calls `MarshalParameters`, asserts the returned error wraps `"unknown MigrationPolicy 99"`.
- `TestMarshalParameters_RejectsUnknownEnum_ArtifactEncryption` — parallel, with an out-of-range `ArtifactEncryption` value.
- `TestMarshalParameters_RejectsUnknownEnum_GrantAuthorizationMode` — parallel.
- `TestMarshalParameters_RejectsUnknownEnum_OverrideThreshold` — parallel.
- `TestExtract_RejectsUnknownEnum_MigrationPolicy` constructs a JSON payload with `"migration_policy": "not-a-policy"`, calls `Extract`, asserts the returned error wraps `"unknown migration_policy"`. The JSON-side coverage is symmetric to the Marshal-side tests above: Marshal tests confirm the SDK refuses to emit bad enums; Extract tests confirm the SDK refuses to accept bad enums on the wire.
- `TestExtract_RejectsUnknownEnum_ArtifactEncryption` — parallel.
- `TestExtract_RejectsUnknownEnum_GrantAuthorizationMode` — parallel.
- `TestExtract_RejectsUnknownEnum_OverrideThreshold` — parallel.

Plus two sentinel-discipline tests that pin the two sentinel round-trips as named regressions rather than implicit coverage inside the round-trip table:

- `TestRoundTrip_UnsetMigrationPolicy_EmitsEmptyStringSentinel` constructs `types.SchemaParameters{}` with `MigrationPolicy` left at its zero value, calls `MarshalParameters`, asserts the JSON output contains `"migration_policy": ""`, then calls `Extract` on that output and asserts the round-tripped `MigrationPolicy` is the zero value. Fails if a future refactor drops the empty-string sentinel or changes its semantics.
- `TestRoundTrip_NilCredentialValidityPeriod_EmitsNegativeOneSentinel` constructs `types.SchemaParameters{CredentialValidityPeriod: nil}`, calls `MarshalParameters`, asserts the JSON output contains `"credential_validity_period": -1`, then round-trips through Extract and asserts the result is nil. Fails if the sentinel constant changes, the mirrored Extract branch drops the sentinel check, or Marshal starts emitting `0` for nil.

Ten tests total. Each has a single clear assertion. Each fails for one specific reason if the guarantee it pins regresses. None require infrastructure beyond `testing` and `encoding/json`.

**Closure proof.** `TestComputePREGrantSplitID_GoldenVector` passes. `TestComputePREGrantSplitID_NFCEdgeCase` passes (pins that NFC and NFD forms of the same visual string produce different SplitIDs). `TestComputePREGrantSplitID_CIDAlgorithmBinding` passes (pins that the same 32-byte digest under different algorithm tags produces different SplitIDs). `TestComputeEscrowSplitID_GoldenVector` passes with the new construction. `TestSplitV2_GoldenVector` regenerated and passes. `grep -rn 'ORTHOLOG-V7.75-ESCROW-SPLIT' --include='*.go'` returns hits only inside `vss_v2.go` and `crypto/hash.go` godoc. `grep -rn 'EscrowSplitDST' --include='*.go'` returns zero hits. `grep -rn 'json.Marshal(map\[string\]any{.*cosignature_threshold' --include='*.go'` returns zero hits. Four `TestMarshalParameters_RejectsUnknownEnum_*` tests pass. Four `TestExtract_RejectsUnknownEnum_*` tests pass. `TestRoundTrip_UnsetMigrationPolicy_EmitsEmptyStringSentinel` and `TestRoundTrip_NilCredentialValidityPeriod_EmitsNegativeOneSentinel` pass. Full `go test ./...` green.

**Verification for Group 2.** Both SplitID derivations route through `LengthPrefixed`. Two golden vectors locked. Zero references to the legacy raw-concat construction. Schema parameters v7.5 fail-closed discipline has named test coverage on both Marshal and Extract sides for all four enums. Both sentinel semantics (empty-string for unset `MigrationPolicy`, `-1` for nil `CredentialValidityPeriod`) have named regression tests that fail specifically if the sentinel discipline is broken.
---
