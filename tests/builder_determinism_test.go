...
func TestBuilderCommitment_MatchesMutations(t *testing.T) {
	...
	if commitment.MutationCount != uint32(len(result.Mutations)) {
		t.Fatal("mutation count mismatch")
	}
...
func TestBuilderDeterminism_1000Entries(t *testing.T) {
	...
	if r1.PathACounts != r2.PathACounts {
		t.Fatal("PathA counts differ")
	}
	if r1.CommentaryCounts != r2.CommentaryCounts {
		t.Fatal("Commentary counts differ")
	}
	if r1.NewLeafCounts == 0 {
		t.Fatal("expected some leaves")
	}
	if r1.CommentaryCounts == 0 {
		t.Fatal("expected some commentary")
	}
