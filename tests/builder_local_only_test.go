...
func TestBuilderLocal_ForeignTargetRoot(t *testing.T) {
	h := newHarness()
	entry, _ := makeEntry(t, envelope.ControlHeader{SignerDID: "did:example:alice", TargetRoot: ptrTo(foreignPos(1)), AuthorityPath: sameSigner()}, nil)
...
func TestBuilderLocal_ForeignDelegationPointer(t *testing.T) {
	...
	entry, _ := makeEntry(t, envelope.ControlHeader{SignerDID: "did:example:delegate", TargetRoot: ptrTo(rootPos), AuthorityPath: delegation(), DelegationPointers: []types.LogPosition{foreignPos(2)}}, nil)
...
func TestBuilderLocal_ForeignScopePointer(t *testing.T) {
	...
	entry, _ := makeEntry(t, envelope.ControlHeader{SignerDID: "did:example:judge", TargetRoot: ptrTo(rootPos), AuthorityPath: scopeAuth(), ScopePointer: ptrTo(foreignPos(2))}, nil)
...
func TestBuilderLocal_LocalPositions(t *testing.T) {
	...
	entry, _ := makeEntry(t, envelope.ControlHeader{SignerDID: "did:example:alice", TargetRoot: ptrTo(rootPos), AuthorityPath: sameSigner()}, nil)
