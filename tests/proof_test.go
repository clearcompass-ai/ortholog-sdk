...
func TestProof_KeyDerivation(t *testing.T) {
	...
	k1 := smt.DeriveKey(p)
	k2 := smt.DeriveKey(p)
	if k1 != k2 { t.Fatal("same position should produce same key") }
	p2 := types.LogPosition{LogDID: "did:example:log", Sequence: 43}
	if k1 == smt.DeriveKey(p2) { t.Fatal("different positions should produce different keys") }
}
...
func TestProof_MembershipVerify(t *testing.T) {
	...
	if err != nil || proof == nil { t.Fatalf("GenerateMembershipProof: %v", err) }
	if err := smt.VerifyMembershipProof(proof, root); err != nil { t.Fatalf("Verify: %v", err) }
}

func TestProof_CorruptSiblingFails(t *testing.T) {
	tree := smt.NewTree(smt.NewInMemoryLeafStore(), smt.NewInMemoryNodeCache())
	p1 := pos(1); k1 := smt.DeriveKey(p1)
	tree.SetLeaf(k1, types.SMTLeaf{Key: k1, OriginTip: p1, AuthorityTip: p1})
	p2 := pos(2); k2 := smt.DeriveKey(p2)
	tree.SetLeaf(k2, types.SMTLeaf{Key: k2, OriginTip: p2, AuthorityTip: p2})
	root, _ := tree.Root()
	proof, _ := tree.GenerateMembershipProof(k1)
	if len(proof.Siblings) == 0 { t.Fatal("expected non-default siblings") }
	for k := range proof.Siblings { proof.Siblings[k] = [32]byte{0xFF}; break }
	if err := smt.VerifyMembershipProof(proof, root); err == nil { t.Fatal("corrupt should fail") }
}

func TestProof_EmptyTreeRoot(t *testing.T) {
	tree := smt.NewTree(smt.NewInMemoryLeafStore(), smt.NewInMemoryNodeCache())
	root, _ := tree.Root()
	if root != smt.DefaultHash(smt.TreeDepth) { t.Fatal("empty tree root should be default hash") }
}
...
func TestProof_NonMembershipAndStaleness(t *testing.T) {
	...
	p1 := pos(1); k1 := smt.DeriveKey(p1)
	tree.SetLeaf(k1, types.SMTLeaf{Key: k1, OriginTip: p1, AuthorityTip: p1})
	p2 := pos(999); k2 := smt.DeriveKey(p2)
	rootBefore, _ := tree.Root()
	proof, err := tree.GenerateNonMembershipProof(k2)
	if err != nil || proof == nil { t.Fatal("GenerateNonMembershipProof failed") }
	if err := smt.VerifyNonMembershipProof(proof, rootBefore); err != nil { t.Fatalf("should verify: %v", err) }
	tree.SetLeaf(k2, types.SMTLeaf{Key: k2, OriginTip: p2, AuthorityTip: p2})
	rootAfter, _ := tree.Root()
	if rootBefore == rootAfter { t.Fatal("root should change") }
	if err := smt.VerifyNonMembershipProof(proof, rootAfter); err == nil { t.Fatal("stale proof should fail") }
}
...
func TestProof_MerkleInclusion(t *testing.T) {
	...
	if err := smt.VerifyMerkleInclusion(proof, head.RootHash); err != nil { t.Fatalf("should verify: %v", err) }
}
...
func TestProofSize_Batch5Clustered(t *testing.T) {
	...
	keys := make([][32]byte, 5)
	for i := 0; i < 5; i++ {
		p := pos(uint64(i + 1)); k := smt.DeriveKey(p); keys[i] = k
		tree.SetLeaf(k, types.SMTLeaf{Key: k, OriginTip: p, AuthorityTip: p})
	}
	proof, err := tree.GenerateBatchProof(keys)
	if err != nil || proof == nil { t.Fatal("GenerateBatchProof failed") }
	if len(proof.Entries) != 5 { t.Fatalf("entries: got %d, want 5", len(proof.Entries)) }
...
func TestProofSize_Batch5VsIndividual(t *testing.T) {
	...
	keys := make([][32]byte, 5)
	for i := 0; i < 5; i++ {
		p := pos(uint64(i*1000 + 1)); k := smt.DeriveKey(p); keys[i] = k
		tree.SetLeaf(k, types.SMTLeaf{Key: k, OriginTip: p, AuthorityTip: p})
	}
	// Batch proof
	...
	for _, k := range keys {
		...
		if p != nil { individualTotal += estimateSingleProofSize(p) }
	}
...
func TestProofSize_StaleAfterUpdate(t *testing.T) {
	tree := smt.NewTree(smt.NewInMemoryLeafStore(), smt.NewInMemoryNodeCache())
	p := pos(1); k := smt.DeriveKey(p)
	tree.SetLeaf(k, types.SMTLeaf{Key: k, OriginTip: p, AuthorityTip: p})
	rootBefore, _ := tree.Root()
	proof, _ := tree.GenerateMembershipProof(k)
	if err := smt.VerifyMembershipProof(proof, rootBefore); err != nil { t.Fatal(err) }
	tree.SetLeaf(k, types.SMTLeaf{Key: k, OriginTip: pos(2), AuthorityTip: p})
	rootAfter, _ := tree.Root()
	if rootBefore == rootAfter { t.Fatal("root should change") }
	if err := smt.VerifyMembershipProof(proof, rootAfter); err == nil { t.Fatal("stale proof should fail") }
}
...
func TestProofSize_100LeavesUniqueKeys(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if seen[k] { t.Fatalf("duplicate key at %d", i) }
		seen[k] = true
	}
}
...
func TestDerivationCommitment_MatchesMutations(t *testing.T) {
	...
	if len(mutations) != 1 { t.Fatalf("mutations: got %d", len(mutations)) }
	if mutations[0].LeafKey != k { t.Fatal("mutation key mismatch") }
}
...
func TestDerivationCommitment_ReplayConsistent(t *testing.T) {
	...
	if commitment.PriorSMTRoot != rootBefore { t.Fatal("prior root mismatch") }
	if commitment.PostSMTRoot != rootAfter { t.Fatal("post root mismatch") }
	if commitment.MutationCount != 2 { t.Fatalf("mutation count: got %d", commitment.MutationCount) }
}
...
func TestDerivationCommitment_EmptyBatch(t *testing.T) {
	...
	tree.StartTracking()
	mutations := tree.StopTracking()
	if len(mutations) != 0 { t.Fatalf("expected 0, got %d", len(mutations)) }
}
