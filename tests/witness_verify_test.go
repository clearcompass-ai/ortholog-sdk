...
func TestWitnessVerify_ECDSA_K3N5(t *testing.T) {
	...
	if err != nil { t.Fatalf("K=3/N=5 should pass: %v", err) }
	if result.ValidCount != 3 { t.Fatalf("valid count: got %d, want 3", result.ValidCount) }
	_, err = signatures.VerifyWitnessCosignatures(cosigned, witnessKeys, 4, nil)
	if err == nil { t.Fatal("K=4 should fail with only 3 signatures") }
}
...
func TestWitnessVerify_BLSMock(t *testing.T) {
	...
	if err == nil { t.Fatal("BLS without verifier should error") }
	...
	if err != nil { t.Fatalf("mock BLS should pass: %v", err) }
	if result.ValidCount != 3 { t.Fatalf("valid: got %d, want 3", result.ValidCount) }
}
